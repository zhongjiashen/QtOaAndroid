...
public class Day {
     ...
     * 文本字体的颜色
     */
    public int textClor= 0xFF5676FC;
    /**
     * 日期背景的类型 0代表无任何背景，1代表当前日期，2代表所选日期，3代表即是当前日期，也被选中
     */
    ...
    public void drawDays(Canvas canvas, Context context, Paint paint) {
        //取窄的边框为圆的半径
        backgroundR = DensityUtils.dp2px(context,29);;
        //画背景
    ...
    private void drawWorkState(Canvas canvas, Paint paint) {
        ...
        switch (workState) {
            case 1:
                paint.setColor(0xFF5676FC);
                break;
            case 2:
                paint.setColor(0xFF9556FC);
                break;
            case 3:
                paint.setColor(0xFFFF6464);
                break;
            case 4:
                paint.setColor(0xFFFC9756);
                break;
        ...
        canvas.drawCircle(cx, cy, backgroundR/2, paint);
    }
    ...
    private void drawTaxt(Canvas canvas, Paint paint) {
        ...
        float x = location_x * width + (width) / 2;
        float y = location_y * height +textHeight+height/2;
        if(backgroundStyle==2||backgroundStyle==3){
...
//        float y = location_y * height + (height + textSize) / 2;
//        canvas.drawText(text, x, y, paint);
    }
    ...
    private void drawBackground(Canvas canvas, Paint paint) {
        ...
        if (backgroundStyle == 1||backgroundStyle==3) {
            paint.setColor(0x555676fc);
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(5);
            //计算圆心的位置
            float cx = location_x * width + width / 2;
            float cy = location_y * height + height / 2;
            canvas.drawCircle(cx, cy, backgroundR/2+5, paint);
            return;
        }
